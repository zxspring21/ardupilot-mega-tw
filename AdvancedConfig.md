...
絕大多數用戶可以使用CLI進行基本設置。但是 APM\_Configh.h 文件中還有一些額外選項。 下面列表中所有項都是默認值或可選值。

要修改默認值或開啟某個特性，從下面的代碼中拷貝相應的代碼行（根據需要保持或修改值），粘貼到 `APM_Config.h` 文件中，然後將代碼上傳到 APM 板中。你必須去掉代碼行前面的 {{//}}}，因為在Arduino中 `//` 表示注視.

**這個配置文件以及如何設置常用選項的說明在[這裡](ConfigGuide.md)。**

一次不要修改太多選項；測試每次修改的效果。

不要拷貝整個 APM\_Config.h.reference 文件 - 可選項集合可能隨軟件的進化而改變，因此最好將你的修改隔離出來。不要修改 config.h 文件; 該文件記錄了在 APM\_Config.h 中沒有修改的選項的默認值，必須與其他 APM 代碼保持同步.

這是 APM\_Config.h.reference 文件的一個副本——注意你的Sketchbook中的版本可能已經更新。
```
//
